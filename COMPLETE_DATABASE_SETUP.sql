-- COMPLETE DATABASE SETUP FOR AI CALLING V3
-- Execute this SQL script in your Supabase dashboard to complete the database setup
-- This includes ALL missing tables and schema updates needed for the system

-- ============================================================================
-- 1. CREATE MISSING CORE TABLES
-- ============================================================================

-- Create campaigns table (main campaign management)
...
-- Create campaign_leads table (leads for campaigns)
...
-- Create dnc_entries table (Do Not Call registry)
...
-- Create appointments table (scheduled appointments from calls)
...
-- Create webhook_endpoints table (webhook configuration)
...
-- Create webhook_deliveries table (webhook delivery logs)
...
-- Create subscriptions table (billing and subscription management)
...
-- Create usage_records table (usage tracking for billing)
...
-- Create compliance_reports table (compliance and regulatory reports)
...
-- Create system_status table (system health monitoring)
...
-- Update ai_agents table to match TypeScript interface
...
-- Update call_logs table to match TypeScript interface
...
-- Webhook deliveries policies (read-only)
...
-- System status policies (read-only for all authenticated users)
CREATE POLICY "Authenticated users can view system status" ON system_status FOR SELECT USING (auth.uid() IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_campaign_leads_next_call_at ON campaign_leads(next_call_at);
...
INSERT INTO system_status (service_name, status, message) VALUES
('api', 'operational', 'API service is running normally'),
('calls', 'operational', 'Call service is running normally'),
('webhooks', 'operational', 'Webhook service is running normally'),
('database', 'operational', 'Database is running normally'),
('ai', 'operational', 'AI service is running normally'),
('analytics', 'operational', 'Analytics service is running normally')
ON CONFLICT DO NOTHING;

-- Success message
DO $$
BEGIN
    RAISE NOTICE 'AI Calling V3 database setup completed successfully!';
